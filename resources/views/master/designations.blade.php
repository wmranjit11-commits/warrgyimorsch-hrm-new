        ...
        <div class="col-xl-4 col-lg-5">
            <div class="card border-0 shadow-sm" style="border-radius: 12px; background: white;">
                <div class="card-header bg-white border-bottom py-3" style="border-radius: 12px 12px 0 0;">
                    <h6 class="fw-bold mb-0 text-uppercase" style="color: #64748b; font-size: 11px; letter-spacing: 0.5px;">Add Designation</h6>
                </div>
                <div class="card-body p-4">
                    <form action="{{ route('master.designation.store') }}" method="POST">
                        @csrf
                        <div class="mb-3">
                            <label class="form-label fw-bold small text-muted text-uppercase mb-2">Designation Name <span class="text-danger">*</span></label>
                            <input type="text" name="name" class="form-control border-0 bg-light shadow-none fw-bold" 
                                placeholder="e.g. Frontend Developer" required style="border-radius: 10px; height: 48px; font-size: 14px;">
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="short_name" class="form-control border-0 bg-light shadow-none fw-bold" 
                                placeholder="e.g. Dev" style="border-radius: 10px; height: 48px; font-size: 14px;">
                        </div>
                ...
                <div class="card-header bg-white border-bottom py-3 d-flex justify-content-between align-items-center"
                    ...
                    <div class="input-group" style="width: 250px;">
                        ...
                        <input type="text" id="desgSearch" class="form-control bg-light border-0 shadow-none fw-bold"
                            placeholder="Search..." onkeyup="filterDesg()" style="height: 44px; font-size: 14px; border-radius: 0 10px 10px 0;">
                    </div>
                </div>
                        ...
                        <table class="table align-middle table-hover mb-0" id="desgTable">
                            <thead style="background: #3858f9; color: white;">
                                <tr style="height: 60px; vertical-align: middle;">
                                    ...
                                    <th style="font-size: 12px; font-weight: 700; color: white; text-transform: uppercase;">Designation Name</th>
                            ...
                            <tbody style="border-top: 1px solid #f1f5f9;">
                                ...
                                <tr class="desg-row" style="height: 70px;">
                                    ...
                                    <td class="fw-bold" style="color: #3858f9; font-size: 14px;">{{ strtoupper($desg->name) }}</td>
                                    <td style="font-size: 14px;">{{ $desg->short_name ?? '-' }}</td>
                                    <td class="pe-4 text-center">
                                        <div class="d-flex justify-content-center gap-2">
                                            <button type="button" onclick="editDesg({{ $desg }})" class="avatar-text avatar-md bg-soft-info text-info rounded border-0" title="Edit">
                                                <i class="feather-edit-3"></i>
                                            </button>
                                            <form action="{{ route('master.designation.destroy', $desg->id) }}" method="POST" onsubmit="return confirm('Are you sure?');">
                                                @csrf @method('DELETE')
...
<div class="offcanvas offcanvas-end shadow-lg" tabindex="-1" id="editDesgModal" style="border-left: none; width: 400px;">
    ...
    <form id="editDesgForm" method="POST" class="d-flex flex-column h-100">
        ...
        <div class="offcanvas-body p-4 bg-light flex-grow-1">
            <div class="mb-3">
                ...
                <input type="text" name="name" id="editDesgName" class="form-control" required style="height: 48px; border-radius: 8px;">
            </div>
            <div class="mb-3">
                <label class="form-label fw-bold text-muted small text-uppercase mb-2">Short Name (Optional)</label>
                <input type="text" name="short_name" id="editDesgShortName" class="form-control" style="height: 48px; border-radius: 8px;">
            </div>
        </div>
